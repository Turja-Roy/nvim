...
K('n', '<leader>dq', vim.diagnostic.setloclist, { desc = 'Open diagnostics list' })

-- Quarto keybindings (global - also see after/ftplugin/quarto.lua for buffer-local)
K('n', '<leader>qh', ':h quarto<CR>', { desc = 'Quarto help', silent = true })
K('n', '<leader>qq', ':ToggleTerm<CR>pkill -f "quarto preview"<CR>exit<CR>', { desc = 'Quit Quarto preview', silent = true })
